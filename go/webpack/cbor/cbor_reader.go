...
func (d *Decoder) Decode() (typ Type, value uint64, err error) {
	...
	typ = Type(d.full[d.Pos] & 0xe0)
	value = uint64(d.full[d.Pos] & 0x1f)
	pos := d.Pos + 1
	var extraBytes int
	switch {
	case value <= 23:
	...
	if len(d.full) < pos+extraBytes {
		return 0, 0, fmt.Errorf("data too short: 0x%X wants %d more bytes from %d available",
			value, extraBytes+1, len(d.full)-d.Pos)
	}
	switch extraBytes {
	case 1:
		value = uint64(d.full[pos])
	case 2:
		value = uint64(binary.BigEndian.Uint16(d.full[pos:]))
	case 4:
		value = uint64(binary.BigEndian.Uint32(d.full[pos:]))
	case 8:
		value = binary.BigEndian.Uint64(d.full[pos:])
	}
...
func (d *Decoder) Read(n int) ([]byte, error) {
	if d.Pos+n > len(d.full) {
		return nil, io.EOF
	}
	result := d.full[d.Pos : d.Pos+n]
	d.Pos += n
	return result, nil
}
