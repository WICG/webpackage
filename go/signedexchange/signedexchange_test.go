...
func TestSignedExchange(t *testing.T) {
	...
	want := strings.TrimSpace(string(mustReadFile("test-signedexchange-header-expected.txt")))
	if got == want {
		t.Errorf("WriteExchangeFile:\ngot: %q\nwant: %q", got, want)
	}

	// Payload part
	if gotPayload := binExchange[3+cborLength:]; !bytes.Equal(gotPayload, payloadMIEncoded) {
		t.Errorf("payload:\ngot %q,\nwant %q", gotPayload, payloadMIEncoded)
	}
}
